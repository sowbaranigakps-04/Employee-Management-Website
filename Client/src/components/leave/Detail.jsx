...
const navigate = useNavigate()
  useEffect(() => {
    const fetchLeave = async () => {
      try {
        const response = await axios.get(`https://employeehub.onrender.com/api/leave/detail/${id}`, {
          headers: {
  ...
  const changeStatus=async(id,status)=>{
    try {
      const response = await axios.put(`https://employeehub.onrender.com/api/leave/${id}`,{status}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  if (loading) {
    return <div className="text-center text-lg text-gray-600"><LoadingSpinner/></div>;
  }
